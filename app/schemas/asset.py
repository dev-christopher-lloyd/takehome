...
class AssetUploadRequest(BaseModel):
    """
    Incoming payload for uploading an asset image.

    Used by:
      - POST /assets

    The image must be base64-encoded. You may optionally include a data URL
...
class AssetMetadata(BaseModel):
    """
    Lightweight view of an asset returned via the API.
    Used by:
      - GET /assets/{asset_id}
    ...
    product_id: Optional[int] = Field(
        None,
        description="ID of the product this asset is associated with (if any).",
    )
